...
sections:
  ...
  - type: comparison_table
    ...
    rows:
      ...
      - label: "Cost"
        values:
          - "Pay per task ($0.001-0.10, models included)"
...
On cost: Open WebUI is free software, but running it requires hosting, compute for local models (a capable GPU is not cheap), and API keys if you connect to cloud providers. LikeClaw uses pay-per-task pricing from $0.001 to $0.10 per task with models included — no subscription required. The total cost of ownership depends entirely on your setup.
